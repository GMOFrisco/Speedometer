...
// ---------------------------------
	uint8_t printChar(uint8_t x, uint8_t row, uint8_t ch) {
		...
		for (i = 0; i < wdL; i++)
			i2c_write(0);
		for (i = 0; i < wd; i++)
			i2c_write(pgm_read_byte(font + idx + i));
		for (i = 0; i < wdR; i++)
			i2c_write(0);
		i2c_stop();
#endif
...
// ---------------------------------
	void init() {
		...
		Serial.println(
				i2c_init() ?
						F("SoftI2C initialization done") :
						F("SoftI2C initialization error. SDA or SCL are low"));
#endif
...
// ---------------------------------
	 ...
	 */
// ---------------------------------
	void drawBitmap(const uint8_t *bmp, uint8_t colS, uint8_t rowS, uint8_t wd,
			uint8_t ht) {
